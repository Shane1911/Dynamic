...
%求摩擦力和摩擦力矩
%求椭圆参数
R1=1e-3*[7.8592 5.1433];%m
a=(6*k.^2.*deta1.*Q_12.*R1/(pi*E)).^(1/3);%椭圆长轴 m
b=a./k;%椭圆短半轴 m
W_j=W(1:4);
ymax1=@(x)a(1)*sqrt(1-(x/b(1)).^2);
ymax2=@(x)a(2)*sqrt(1-(x/b(2)).^2);
fox=4*integral2(@(x,y)fun3(x,y,W_j,a,b,1),0,b(1),0,ymax1);
% c1=fox/Q_1j
% foy=4/hc(1)*integral2(@(x,y)fun3(x,y,W_j,a,b,2),0,b(1),0,ymax1);
fix=4*integral2(@(x,y)fun3(x,y,W_j,a,b,3),0,b(2),0,ymax2);
% c2=fix/Q_2j
% fiy=4/hc(2)*integral2(@(x,y)fun3(x,y,W_j,a,b,4),0,b(2),0,ymax2);
% mo=4/hc(1)*integral2(@(x,y)fun3(x,y,W_j,a,b,5),0,b(1),0,ymax1);
% mi=4/hc(2)*integral2(@(x,y)fun3(x,y,W_j,a,b,6),0,b(2),0,ymax2);
% f=[0.519 0.519];%沟道曲率
% Ro=2*f(1)*D*1e-3/(2*f(1)+1);
% ro=(Ro^2)^0.5-(Ro^2-a(1)^2)^0.5+((0.5*1e-3*D)^2-a(1)^2)^0.5;
% Ri=2*f(2)*D*1e-3/(2*f(2)+1);
% ri=(Ri^2)^0.5-(Ri^2-a(2)^2)^0.5+((0.5*1e-3*D)^2-a(2)^2)^0.5;
% uox=0.5*W(4)*1e-3*(dm)-ro*(W(1)*cos_1j+W(3)*sin_1j...
%     -W(4)*cos_1j);
% uix=0.5*1e-3*(W(4)-omega_i)*(dm)+ri*(W(1)*cos_2j+W(3)...
%     *sin_2j-(W(4)-omega_i)*cos_2j);
% us=[abs(uox) abs(uix)];
% SSR1=2*uox/u(1)
% SSR2=2*uix/u(2)
Y=[fox fix];
