...
E1=2.075e11;
E=E1/(1-0.3^2);
yita_0=0.04667;  % changed items
alfa_0=1.2*1e-8;         % changed items
U=yita_0.*u./(2*E.*Rx);
G=E*alfa_0;
W_l=Q_12./(E.*Rx.^2);
k=[7.4814 9.8926];
...
Icage=4.096e-5;
V1=0.5*53*1e-3*(omega_i-W(4));
FM_cage=2*pi*yita_0*1e-3*(0.5*53)^2*V1*15/0.15/Zn/(0.5*dm);
Tao=diag([I I I Ic 1 Ic 1]);
MF=[];
MF(1)=Mx+I*W(4)*W(2);
MF(2)=My-I*W(4)*W(1);
MF(3)=Mz-0.02*FM_cage*0.5*D*1e-3;
MF(4)=0.5*1e-3*(-70.1*fox+43.1*fix)-0.5*1e-3*dm*(f_cage+f_v+FM_cage);
MF(5)=W(4);
